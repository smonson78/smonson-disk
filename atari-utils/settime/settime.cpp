...
int test_inquiry_command() {

    uint8_t cmdbuf[6];
    uint8_t dmabuf[512];

    uint8_t controller = 0;
    uint8_t command = 0x12; // inquiry

    memset(cmdbuf, 0, 6);
    cmdbuf[0] = (controller << 5) | command;
    cmdbuf[3] = 36; // buffer size

    acsi_command(cmdbuf, 6, dmabuf, 1, 0);
...
int test_read_command() {
    ...
    uint8_t controller = 0;
    uint8_t command = 0x8; // read 6
    uint32_t sector = 0;
    uint8_t length = 1;

    memset(cmdbuf, 0, 6);
    cmdbuf[0] = (controller << 5) | command;
    cmdbuf[1] = (sector >> 16) & 0xff;
    cmdbuf[2] = (sector >> 8) & 0xff;
    cmdbuf[3] = sector & 0xff;
    cmdbuf[4] = length;

    acsi_command(cmdbuf, 6, dmabuf, 36, 0);

    return 0;
}

int test_read_invalid_sector() {
...
int main(int argc, char **argv)
{
    ...
    printf("ACSI command test\n");

    //xbios_supexec(test_inquiry_command);
    xbios_supexec(test_read_invalid_sector);
